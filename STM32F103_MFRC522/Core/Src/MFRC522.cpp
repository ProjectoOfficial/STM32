 ...
 * Constructor.
 */
MFRC522::MFRC522(SPI_HandleTypeDef *spi): MFRC522(spi, nullptr, UINT16_MAX, nullptr, UINT16_MAX) {
} // End constructor
 ...
 */
MFRC522::MFRC522(SPI_HandleTypeDef *spi, GPIO_TypeDef* ResetPort, uint16_t ResetPin): MFRC522(spi, nullptr, UINT16_MAX, ResetPort, ResetPin) {
} // End constructor
 ...
 * Prepares the output pins.
 */
MFRC522::MFRC522(SPI_HandleTypeDef *spi, GPIO_TypeDef* CSPort, uint16_t CSPin, GPIO_TypeDef* ResetPort, uint16_t ResetPin) {
	hspi = spi;
	CS_Port = CSPort;
	CS = CSPin;
	RST_Port = ResetPort;
...
void MFRC522::PCD_WriteRegister(PCD_Register reg, uint8_t count, uint8_t *values) {
	...
	for (uint8_t index = 0; index < count; index++) {
		HAL_SPI_Transmit(hspi, &values[index], sizeof(values[index]), HAL_MAX_DELAY);
	}
	HAL_GPIO_WritePin(CS_Port, CS, GPIO_PIN_SET);
} // End PCD_WriteRegister()
...
uint8_t MFRC522::PCD_ReadRegister(PCD_Register reg) {
	...
	HAL_SPI_Transmit(hspi, &address, sizeof(address), HAL_MAX_DELAY);

	uint8_t result;
	HAL_SPI_TransmitReceive(hspi, &address, &result, sizeof(result), HAL_MAX_DELAY);
	HAL_SPI_Transmit(hspi, 0, 1, HAL_MAX_DELAY);
	HAL_GPIO_WritePin(CS_Port, CS, GPIO_PIN_SET);
...
void MFRC522::PCD_ReadRegister(PCD_Register reg, uint8_t count, uint8_t *values, uint8_t rxAlign) {
	...
	if (rxAlign) {		// Only update bit positions rxAlign..7 in values[0]
		...
		uint8_t value;
		HAL_SPI_TransmitReceive(hspi, &address, &value, sizeof(value), HAL_MAX_DELAY);
	...
	while (index < count) {
		uint8_t tmp;
		HAL_SPI_TransmitReceive(hspi, &address, &tmp, sizeof(tmp), HAL_MAX_DELAY);;	// Read value and tell that we want to read the same address again.
		values[index] = tmp;
		index++;
	}

	uint8_t last_val;
	HAL_SPI_TransmitReceive(hspi, &address, &last_val, sizeof(last_val), HAL_MAX_DELAY); // Read the final uint8_t. Send 0 to stop reading.
...
void MFRC522::pinMode(GPIO_TypeDef *GPIO_Port, uint16_t GPIO_Pin, PinModes mode){
...
void MFRC522::PCD_Init() {
	...
	pinMode(CS_Port, CS, OUTPUT);
	HAL_GPIO_WritePin(CS_Port, CS, GPIO_PIN_SET);

	// If a valid pin number has been set, pull device out of power down / reset state.
	if ((RST_Port != UNUSED_PORT) && (RST != UNUSED_PIN)) {
		// First set the resetPowerDownPin as digital input, to check the MFRC522 power down mode.
		pinMode(RST_Port, RST, INPUT);

        GPIO_PinState reset = HAL_GPIO_ReadPin(RST_Port, RST);
		if (reset == GPIO_PIN_RESET) {			// The MFRC522 chip is in power down mode.
			pinMode(RST_Port, RST, OUTPUT);				// Now set the resetPowerDownPin as digital output.
			HAL_GPIO_WritePin(RST_Port, RST, GPIO_PIN_RESET); 	// Make sure we have a clean LOW state.
			HAL_Delay(2);										// 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2μsl
			HAL_GPIO_WritePin(RST_Port, RST, GPIO_PIN_SET);		// Exit power down mode. This triggers a hard reset.
 ...
 * Initializes the MFRC522 chip.
 */
void MFRC522::PCD_Init(SPI_HandleTypeDef *spi, GPIO_TypeDef *ResetPort, uint16_t ResetPin) {
	PCD_Init(spi, UNUSED_PORT, UNUSED_PIN, ResetPort, ResetPin); // SS is defined in pins_arduino.h
} // End PCD_Init()

/**
 * Initializes the MFRC522 chip.
 */
void MFRC522::PCD_Init(SPI_HandleTypeDef *spi, GPIO_TypeDef *CSPort, uint16_t CSPin, GPIO_TypeDef *ResetPort, uint16_t ResetPin) {
	hspi = spi;
	CS_Port = CSPort;
	CS = CSPin;
	RST_Port = ResetPort;
...
bool MFRC522::PCD_PerformSelfTest() {
	...
	uint8_t version;
	PCD_ReadRegister(VersionReg, 1, &version, 0);
...
MFRC522::StatusCode MFRC522::PICC_Select(	Uid *uid,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
	...
	while (!uidComplete) {
		...
		if (bytesToCopy) {
			...
			if (bytesToCopy > maxuint8_ts) {
				bytesToCopy = maxuint8_ts;
			}
 ...
 */
uint8_t MFRC522::PCD_DumpVersion() {
	// Get the MFRC522 firmware version
	return (uint8_t) PCD_ReadRegister(VersionReg);
